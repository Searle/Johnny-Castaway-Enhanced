...
* **Windowed Mode (`-window` or `-w` [<WxH>])**: Runs in a normal decorated, resizable window instead of the borderless monitor-spanning screensaver window, with the cursor visible and input-exit disabled so the window stays usable. The scene is scaled to fill the window while preserving its 4:3 aspect ratio (pillarboxed with black bars when the window is not 4:3; an exact 4:3 size like `1024x768` fills it edge to edge). An optional size sets the initial window dimensions, e.g. `-window 1280x960` (defaults to `1280x960`); the scene keeps fitting as the window is resized live. On Linux this also avoids the borderless-window sizing quirk under compositors (notably WSLg) that refuse programmatic resizing.
...
* **Coordinate & Scale Fixes**:
  ...
  * Scaled horizontal coordinates of screen-spanning elements (like the biplane flyby in `GJVIS3.TTM` and `GJVIS5.TTM`, the tanker in `GJVIS6.TTM`, and rescue boat in `WOULDBE.TTM`) across the full widescreen viewport width when widescreen mode is enabled. Restricted scaling only to true screen-spanning sprite image slots (e.g. the speedboat or tanker itself) while applying the standard island shift offset (`widescreenOffsetX`) to static island-based sprites in the same TTMs (such as the palm tree trunk overlay or Johnny standing/walking on the island), preventing graphical misalignment.
...
* **MARY.ADS Tag 3 Ghosting / Stalled Exit**: Fixed a scene-control bug where ADS thread ownership checks were matching against the live TTM tag even after a scene entered a `:LOCAL_TAG` loop. Threads now retain their original `ADD_SCENE` tag separately from the current execution tag, so `STOP_SCENE`, `IF_IS_RUNNING`, replay handling, and `IF_LASTPLAYED` dispatch continue to target the correct scene. This resolves the MARY 3 ghost-sprite buildup and the scene getting stuck before its ending transition.
